...
reactRouter.get("/posts/:post_id/reacts", async(req,res) => {
    ...
    try{
        const result = await query('SELECT * FROM post_reacts WHERE post_id = $1', [post_id]);
        const userReacted = await query('SELECT * FROM post_reacts WHERE post_id = $1 AND user_id = $2', [post_id, user_id]);
        const reactions = result.rows ? result.rows : [];
        res.status(200).json({ reactions: reactions.length, userReacted: userReacted.rowCount > 0 });
    } catch (error) {
