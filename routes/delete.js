...
async function deleteCommentReacts(id) {
  const sql =
    "delete from comment_reacts where comment_id in (select comment_id from comments inner join posts on comments.post_id = posts.post_id where posts.user_id = $1)";
  ...
  try {
    ...
    if (data.rowCount === 1) {
      ...
      if (checkPassword) {
        ...
        res
          .status(200)
          .json({ user_id: userId, message: "Account deleted successfully" });
      } else {
